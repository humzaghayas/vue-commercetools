...
export default {
  ...
  setup(_, context) {
    ...
    const { $ct: { config } ,$ct} = useVSFContext();
    ...
    const handleCheckSameAddress = async () => {
      ...
      if (sameAsShipping.value) {
        if (!shippingDetails.value) {
          //await loadShipping();
           cart= await $ct.api.getCart(cartId);
        }
        oldBilling = {...billingDetails.value};
        billingDetails.value = cart.data.cart.shippingAddress;
    ...
    const handleAddressSubmit = (reset) => async () => {
      ...
      reset();
      router.push(`/checkoutquote/payment?quoteId=${cartId}`);
    };
