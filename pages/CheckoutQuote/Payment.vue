...
import { ref, computed, watch, useRouter, useRoute } from '@nuxtjs/composition-api';
...
export default {
  ...
  async asyncData({ app, params,query ,loading,$vsf}){
    ...
    const shippingDetails = data.cart.shippingAddress;
    const billingDetails = data.cart.shippingAddress;
    const products = data.cart.lineItems;
    const billingSameAsShipping =  Object.keys(shippingDetails).every(shippingDetailsKey => shippingDetails[shippingDetailsKey] === billingDetails[shippingDetailsKey]);

    const totals = {
      total: data.cart.totalPrice.centAmount / 100,
      subtotal:  data.cart.totalPrice.centAmount / 100,
      special:  data.cart.totalPrice.centAmount / 100
    };
